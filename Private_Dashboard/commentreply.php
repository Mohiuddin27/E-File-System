    ...
    <div class="sidebar-fixed position-fixed">
      ...
      <a class="logo-wrapper waves-effect">
        ...
        <a href="view_userfile.php" class="list-group-item list-group-item-action waves-effect">
          ...
          <a href="dep.php" class="list-group-item list-group-item-action waves-effect">
          <i class="fas fa-chalkboard-teacher"></i>Department</a>
          <a href="allcomment.php" class="list-group-item list-group-item-action waves-effect">
...
$q = mysqli_query($conn,"select * from comments where id = '$cmt_id'") or die (mysqli_error($conn));

$rs1 = mysqli_fetch_array($q);

              $id1=$rs1['id'];
              $user_comment=$rs1['user_comment'];
              $admin_reply=$rs1['admin_reply'];
...
?>
    ...
    <div class="modal-content" style="width:40%;display:block;margin:0 auto">
      <div class="modal-header text-center">
        <h4 class="modal-title w-100 font-weight-bold"><i class="fas fa-user-edit"></i>Reply Comment</h4>
        
      </div>
        <div class="modal-body mx-3">
           <div class="md-form mb-2">
            <label style="font-size:25px;font-weight:600">User Comment:</label><br><br>
           <textarea style="width:100%"readonly><?php echo $user_comment;?></textarea>
        </div>
        <div class="md-form mb-5">
        <label style="font-size:20px;font-weight:600">Admin Reply:</label><br><br>
          <textarea name="admin_reply" style="width:100%"><?php echo $admin_reply;?></textarea>
        </div>
      </div>
      <div class="modal-footer d-flex justify-content-center">
...
if(isset($_POST['reply'])){
    ...
    mysqli_query($conn,"UPDATE `comments` SET `admin_reply` = '$admin_reply' where id='$cmt_id'") or die (mysqli_error($conn));
 
 echo "<script type = 'text/javascript'>alert('Admin has Replied!');document.location='allcomment.php'</script>";
...
?>
 ...
 if(isset($_POST['edit'])){
     ...
     mysqli_query($conn,"UPDATE `login_user` SET `name` = '$user_name', `email_address` = '$email_address',`department`='$department', `user_password` = '$user_password' where id='$id'") or die (mysqli_error($conn));
